...
- [`self-mcp-reconnect.md`](self-mcp-reconnect.md) — force a stale MCP server's TOOL DISCOVERY to refresh via `self-mcp-reconnect`: drive the interactive `/mcp` picker end-to-end (open the list, select a server, choose "Reconnect", confirm the inline result) for the case where `claude mcp list` says Connected but the session's cached tool list is still stale (typically `mcp-adaptor` after a host-side re-auth, e.g. missing `doc_search` tools). Reuses self-clear's pane primitives; changes neither the binary nor the container.
...
- [`authoring-skills.md`](authoring-skills.md) — meta-skill for writing a NEW skill for this system: SKILL.md/frontmatter authoring basics, plus the baked-vs-linked delivery decision (baked = `container/skills/<name>.md`, needs an image rebuild + recreate; linked = any repo's own `SKILL.md`, installed via `scripts/install-linked-skills.sh` with no rebuild — see [`docs/linked-skills.md`](../../docs/linked-skills.md)).
