...
2. **Trigger the reconnect**: run `self-mcp-reconnect run` inside the container. Common invocations:
   ...
   - `self-mcp-reconnect run --foreground --json` — block and print one JSON result object on stdout. Use this for a PROGRAMMATIC caller (an agent, a cron producer) that wants the outcome synchronously instead of polling the state file. **From inside the very session being targeted, do NOT use `--foreground`** — the flow interrupts the pane's current turn, and without the fork that interrupt would cancel the Bash tool call still waiting on this process. Foreground mode is for an OUT-OF-SESSION caller (`docker compose exec`, a host script).
