...
- claude-watch PRs use the `hndrewaall` gh account; ACME_ORG PRs use `hallandrew_sfemu`. Target per-call via `GH_CONFIG_DIR` — NEVER `gh auth switch` (`feedback_gh_config_dir_isolation`).
