...
1. **Trigger the in-container Claude Code restart**: run `cwsr` (with no flags) inside the container. This:
...
3. **Confirm**: tell the operator the in-container Claude Code restart has been initiated. The current session's claude process will exit; the new one starts in the same pane. Tmux session, MCP bridges, named-volume claude versions/ dir, and the operator's `tmux attach` all survive the roll — only the inner `claude` process changes.

## When `/claude-container:claude-code-restart` (this skill) is NOT the right tool

- **You need an actual CONTAINER restart**: re-running `entrypoint.sh`, re-seeding obligations, picking up changed entrypoint-time env vars / new bind-mounts / a freshly-baked image. This skill does NONE of that — it only rolls the inner `claude` process. Use the sibling `/claude-container:restart-container` skill (which does `docker compose up -d --force-recreate`).
...
- **Need to change entrypoint-time env vars**: `CLAUDE_AUTO_CONTINUE`, `CLAUDE_CONTAINER_REWRITE_HOOKS`, `CLAUDE_HOST_PROJECT_DIR`, `CLAUDE_SHIM_PATTERNS`, etc. are baked at container startup. `cwsr` only rolls the inner process with whatever shape `entrypoint.sh` already chose. Use `/claude-container:restart-container` (or ask the operator to `docker compose up -d --force-recreate`) for those.
- **Want to clear context within the same binary**: that's `/clear`, not this skill. `/clear` is a Claude-Code-internal control-plane action; `cwsr` swaps the binary itself.
