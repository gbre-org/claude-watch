...
`ACME_ORG-sf/ACME_ORG-html-to-pdf` scoped `repo:ACME_ORG` (the monorepo). Both
...
succeeds (rc 0). It must:
  * fire on the clear `ACME_ORG-sf/ACME_ORG-html-to-pdf` scoped `repo:ACME_ORG`
...
def test_the_canonical_mismatch_warns():
    """ACME_ORG-sf/ACME_ORG-html-to-pdf task scoped repo:ACME_ORG -> warn."""
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(
            env,
            "fix pagination in ACME_ORG-sf/ACME_ORG-html-to-pdf renderer",
            ["repo:ACME_ORG"],
        )
        # Warns but still enqueues (rc 0, valid JSON emitted).
        assert r.returncode == 0, (r.returncode, r.stderr)
        assert _WARN_NEEDLE in r.stderr, r.stderr
        assert "ACME_ORG-html-to-pdf" in r.stderr
        d = json.loads(r.stdout)
        assert d["scope"] == ["repo:ACME_ORG"]  # scope unchanged; advisory only


def test_correct_scope_no_warning():
    """Same task correctly scoped repo:ACME_ORG-html-to-pdf -> no warning."""
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(
            env,
            "fix pagination in ACME_ORG-sf/ACME_ORG-html-to-pdf renderer",
            ["repo:ACME_ORG-html-to-pdf"],
        )
...
def test_ambiguous_multi_repo_no_warning():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = _add(
            env,
            "sync shared config between ACME_ORG-typesense and pr-watch",
            ["repo:claude-watch"],
        )
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_star_scope_never_warns():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(env, "touch ACME_ORG-sf/ACME_ORG-html-to-pdf", ["*"])
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_fail_open_when_repos_dir_absent():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = _add(env, "edit ACME_ORG-sf/ACME_ORG-html-to-pdf", ["resource:x"])
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_path_scope_covers_target():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = _add(
            env,
            "edit ACME_ORG-sf/ACME_ORG-html-to-pdf/src/render.ts",
            ["path:ACME_ORG-html-to-pdf/src"],
        )
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_more_specific_scope_covers_bare_parent_mention():
    """scope repo:ACME_ORG-typesense covers a bare 'ACME_ORG' text mention."""
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        # Text names only the specific sub-repo; scope claims it. The prefix
        # 'ACME_ORG' also matches as a dir but collapses to the specific one.
        r = _add(
            env,
            "bump the ACME_ORG-typesense client version",
            ["repo:ACME_ORG-typesense"],
        )
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_home_repos_path_mention_warns():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(env, "edit ~/repos/ACME_ORG-typesense config", ["repo:ACME_ORG"])
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE in r.stderr, r.stderr
        assert "ACME_ORG-typesense" in r.stderr
