...
`regrello-sf/regrello-html-to-pdf` scoped `repo:regrello` (the monorepo). Both
...
succeeds (rc 0). It must:
  * fire on the clear `regrello-sf/regrello-html-to-pdf` scoped `repo:regrello`
...
def _env_for_tmp(tmp, *, make_repos=_REPOS):
    env = dict(os.environ)
    env["HOME"] = str(tmp)
...
def test_the_canonical_mismatch_warns():
    """regrello-sf/regrello-html-to-pdf task scoped repo:regrello -> warn."""
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(
            env,
            "fix pagination in regrello-sf/regrello-html-to-pdf renderer",
            ["repo:regrello"],
        )
        # Warns but still enqueues (rc 0, valid JSON emitted).
        assert r.returncode == 0, (r.returncode, r.stderr)
        assert _WARN_NEEDLE in r.stderr, r.stderr
        assert "regrello-html-to-pdf" in r.stderr
        d = json.loads(r.stdout)
        assert d["scope"] == ["repo:regrello"]  # scope unchanged; advisory only


def test_correct_scope_no_warning():
    """Same task correctly scoped repo:regrello-html-to-pdf -> no warning."""
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(
            env,
            "fix pagination in regrello-sf/regrello-html-to-pdf renderer",
            ["repo:regrello-html-to-pdf"],
        )
...
def test_ambiguous_multi_repo_no_warning():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = _add(
            env,
            "sync shared config between regrello-typesense and pr-watch",
            ["repo:claude-watch"],
        )
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_star_scope_never_warns():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(env, "touch regrello-sf/regrello-html-to-pdf", ["*"])
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_fail_open_when_repos_dir_absent():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = _add(env, "edit regrello-sf/regrello-html-to-pdf", ["resource:x"])
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_path_scope_covers_target():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = _add(
            env,
            "edit regrello-sf/regrello-html-to-pdf/src/render.ts",
            ["path:regrello-html-to-pdf/src"],
        )
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_more_specific_scope_covers_bare_parent_mention():
    """scope repo:regrello-typesense covers a bare 'regrello' text mention."""
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        # Text names only the specific sub-repo; scope claims it. The prefix
        # 'regrello' also matches as a dir but collapses to the specific one.
        r = _add(
            env,
            "bump the regrello-typesense client version",
            ["repo:regrello-typesense"],
        )
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE not in r.stderr, r.stderr
...
def test_home_repos_path_mention_warns():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        env = _env_for_tmp(tmp)
        r = _add(env, "edit ~/repos/regrello-typesense config", ["repo:regrello"])
        assert r.returncode == 0, r.stderr
        assert _WARN_NEEDLE in r.stderr, r.stderr
        assert "regrello-typesense" in r.stderr
