...
operates on (the canonical case: a task editing `regrello-sf/regrello-html-to-pdf`
scoped `repo:regrello` — both real dirs, but the scope over-serializes/races
independent per-repo work). At `queue add` time, when the description/summary
names exactly ONE unambiguous target repo dir (via a `regrello-sf/<repo>` /
...
still enqueues (rc 0).
  ...
  * Coverage: a scoped repo that is the target or *more specific* than it
    (`repo:regrello-typesense` covers a bare `regrello` mention) suppresses
    the warning; a scoped repo *less specific* than the target
    (`repo:regrello` vs target `regrello-html-to-pdf`) is the mismatch.
