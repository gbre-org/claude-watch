...
| `pre-tool-obligations-gate-hook` | PreToolUse | `*` | Calls `obligations check`; denies when a gate-mode obligation's predicate is unsatisfied. Also enforces hardcoded architectural gates: (a) bare-`watcher-ctl run` cardinal rule; (b) `Monitor` tool denied inside subagent context (`agent_id` non-empty); (c) a narrow ALLOW for the heartbeat-liveness commands (a single-segment bare `touch /var/run/claude/heartbeat`, or a bare `heartbeat-ack` — the wrapper that does that touch AND acks the pending `heartbeat-tick` event — passes even when the obligations gate would otherwise DENY, so claude-watch's stale-heartbeat fallback can't be blocked by a momentarily-down watcher) -- see [`docs/hooks.md`](../../docs/hooks.md#hardcoded-architectural-gates). |
