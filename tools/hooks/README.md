...
| `pre-tool-obligations-gate-hook` | PreToolUse | `*` | Calls `obligations check`; denies when a gate-mode obligation's predicate is unsatisfied. Also enforces hardcoded architectural gates: (a) bare-`watcher-ctl run` cardinal rule; (b) `Monitor` tool denied inside subagent context (`agent_id` non-empty); (c) a narrow ALLOW for the heartbeat-liveness touch (a single-segment bare `touch /var/run/claude/heartbeat` passes even when the obligations gate would otherwise DENY, so claude-watch's stale-heartbeat fallback can't be blocked by a momentarily-down watcher; heartbeat-tick's normal clear-path is a plain `event-ack ack`, already exempt from the `event_must_act` evaluator on its own -- see [`docs/hooks.md`](../../docs/hooks.md#hardcoded-architectural-gates)). |
...
| `pre-tool-dispatch-gate-hook` | PreToolUse | `*` | Counts consecutive non-exempt non-Agent tool calls; denies the (N+1)th once the threshold is crossed (default 6). Spawning an Agent (or `claude-watch-dispatch reset`) zeros the counter. Tweakable via `CLAUDE_WATCH_DISPATCH_THRESHOLD`, `CLAUDE_WATCH_DISPATCH_ENABLED=0`, `CLAUDE_WATCH_DISPATCH_BYPASS=1`, and `/etc/claude-code/dispatch-exempt.txt` (one regex per line). Exempt: `Read` + `Agent` tools and an inspection-only Bash list (`ls`, `pwd`, `cat`, `grep`, `rg`, `find` without `-delete`/`-exec`, `git status/diff/log/show/rev-parse/branch/blame`, `session-task`, `claude-watch-ack`, `claude-watch-dispatch`, `obligations list/show/status`, `self-clear`, `agent-msg inbox/list/status`, `agent-tail`, `watcher-ctl run/status`) plus the dispatcher-loop maintenance surface that mirrors the `event_must_act` exempt-set (`agent-ack`, `event-ack`, `event-classify`, and the heartbeat-liveness `touch /var/run/claude/[claude-]heartbeat`). |
...
| `pre-agent-background-required-hook` | PreToolUse | `Agent` | Denies `Agent` spawns whose `run_in_background` is explicitly falsy. Foreground agents block the main loop, defeating the dispatcher model. **Capability-aware:** a spawn with no `run_in_background` key at all is ALLOWED — newer harness builds dropped the parameter and background subagents natively, so demanding it denied 100% of spawns (observed 2026-08-13 on both the host loop and workbot). Env bypass `AGENT_FOREGROUND_OK=1` (audited) or per-prompt marker `FOREGROUND_AGENT_OK: <reason>`. |
